...
have equality of the functions and derivatives in \(\weights\).
\begin{lemma}
	From (\ref{eq: upper lower bound on hesse matrix}) follows
...
\begin{proof}
	Define
	\begin{align*}
		g(\theta)&:=\bregmanDiv{\Loss}(\theta,\weights),\\
		f(\theta)&:=\tfrac\lbound2\|\theta-\weights\|^2, 
	\end{align*}
	then we have for \(h:=g-f\)
	\begin{align*}
		\nabla^2 h(\theta) = \nabla^2 \Loss(\theta) - \lbound\identity\succsim 0
		\quad \forall \theta
	\end{align*}
	due to (\ref{eq: upper lower bound on hesse matrix}). And therefore by
	Taylor expansion with integral remainder, we have
	\begin{align*}
		h(\theta)
		&= \underbrace{h(\weights)}_{=0}
		+ \langle \underbrace{\nabla h(\weights)}_{=0}, \theta-\weights\rangle 
		+ \int_0^1 \langle
		\underbrace{
			\nabla^2 h(\weights + t(\theta-\weights))(\theta-\weights),
			\theta-\weights
		}_{\ge 0}
		\rangle dt\\
		&\ge 0 \quad \forall \theta.
	\end{align*}
	This proves the first inequality. The proof for the second inequality is analogous.
\end{proof}
